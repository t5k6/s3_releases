...
sys_check_headers() {
	...
	for header in "${headervars[@]}"; do
        ...
        if [ "$header" = "dvbcsa.h" ] && [ "${s3cfg_vars[INSTALL_NATIVE_LIBDVBCSA]:-1}" = "0" ]; then
...
local output_mode="${1:-verbose}"  # silent/verbose/summary
    ...
    # Run all checks and accumulate failures
    local bin_failures="0"
    bin_failures=$(sys_check_binaries "$output_mode")
    total_failures=$((total_failures + bin_failures))

    local header_failures="0"
    header_failures=$(sys_check_headers "$output_mode")
    total_failures=$((total_failures + header_failures))

    local lib_failures="0"
    lib_failures=$(sys_check_libraries "$output_mode")
    total_failures=$((total_failures + lib_failures))
