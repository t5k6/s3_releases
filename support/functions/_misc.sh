...
_build_extra_menu(){
	...
	if menu_show_checkbox "17" "45"; then
		...
		for option in "${selected_build_extra[@]}"
		do
...
_gtedit(){
	...
	then
		...
		confdir=$(ui_get_input " -[ $1 Toolchain$(REPOIDENT) ]- " "CONF_DIR=$($_oscamconfdir_custom:=$_oscamconfdir_default) 'default'" "$_oscamconfdir_custom")
